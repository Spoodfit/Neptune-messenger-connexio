...
|---|---|---|
| TypeScript strict | Validé dépôt | `npm run typecheck` dans la CI |
| Tests de domaine | Validé dépôt | `npm test` dans la CI |
| Installation reproductible | En validation | `package-lock.json` committé, gate `npm ci` obligatoire |
...
| Domaine | Statut | Preuve / limite |
|---|---|---|
| Envoi optimiste | Validé dépôt | État local puis réconciliation serveur |
...
| Validation des réponses réseau | Validé dépôt | Sessions, conversations, messages et WebSocket |
| Principe du moindre privilège | Validé dépôt | Rôle inconnu normalisé vers Triton |
