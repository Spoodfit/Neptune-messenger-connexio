...
export default function MemberProfileScreen() {
  ...
  const ensureConversation = async () => {
    ...
    if (!canInitiatePrivateInteraction(currentUser.role)) {
      Alert.alert("Passez Triton", "Un compte Free peut être invité à discuter ou recevoir un appel, mais ne peut pas initier l’échange.", [
  ...
  const contactAbout = (item: BusinessItem) => {
    const firstName = member.name.split(" ")[0] || member.name;
    const subject = item.title?.trim() || member.company || "votre activité";
    const readyMessage = `Bonjour ${firstName}, votre ${item.kind === "product" ? "produit" : item.kind === "service" ? "service" : "activité"} « ${subject} » m’intéresse. Pouvons-nous en parler ?`;
    Alert.alert("Démarrer la mise en relation ?", readyMessage, [
      { text: "Annuler", style: "cancel" },
      {
        text: "Envoyer et discuter",
        onPress: () => {
          void (async () => {
            if (opening) return;
            setOpening(true);
            try {
              ...
              const accepted = conversation.id.startsWith("local-")
                ? await sendLocalMessage(conversation.id, readyMessage)
                : await sendMessage(conversation.id, readyMessage);
              ...
              Alert.alert("Mise en relation impossible", error instanceof Error ? error.message : "Réessayez ultérieurement.");
            } finally { setOpening(false); }
          })();
        }
      }
  ...
  const reportMember = async () => {
    if (!api) { Alert.alert("Signalement enregistré", "Le mode démonstration a simulé l’envoi à la modération."); return; }
    try { await api.reportContent("profile", member.id, "Profil signalé depuis Connexio"); Alert.alert("Signalement transmis", "La modération Neptune va examiner ce profil."); }
    catch (error) { Alert.alert("Signalement impossible", error instanceof Error ? error.message : "Réessayez ultérieurement."); }
  };
  const blockMember = () => Alert.alert(`Bloquer ${member.name} ?`, "Vous ne recevrez plus ses messages et il ne pourra plus vous appeler dans Connexio.", [
    { text: "Annuler", style: "cancel" },
    { text: "Bloquer", style: "destructive", onPress: () => void (async () => { try { if (api) await api.blockMember(member.id); Alert.alert("Membre bloqué"); router.replace("/(tabs)/messages"); } catch (error) { Alert.alert("Blocage impossible", error instanceof Error ? error.message : "Réessayez ultérieurement."); } })() }
  ]);
  const muteMember = async () => {
    try { const conversation = await ensureConversation(); const nextMuted = !conversation.muted; if (api && !conversation.id.startsWith("local-")) { await api.setConversationMuted(conversation.id, nextMuted); await refreshConversations(); } else toggleConversationMuted(conversation.id); Alert.alert(nextMuted ? "Messages mis en sourdine" : "Notifications réactivées"); }
  ...
  const menuOptions: ActionSheetOption[] = [
    ...
    ...(env.mockMode || BACKEND_CAPABILITIES.blockedMembers ? [{ id: "block", label: "Bloquer ce membre", icon: "person-remove-outline", destructive: true, onPress: blockMember }] satisfies ActionSheetOption[] : [])
  ];
  ...
  return (
    <LinearGradient colors={gradients.screen} style={styles.screen}>
      <View style={[styles.header, { paddingTop: Math.max(insets.top, spacing.sm), paddingLeft: spacing.sm + insets.left, paddingRight: spacing.sm + insets.right }]}>
        ...
        <Text style={styles.headerTitle}>Profil membre</Text>
        <Pressable onPress={() => setMenuOpen(true)} style={styles.headerButton}><Ionicons name="ellipsis-horizontal" size={23} color={colors.text} /></Pressable>
      </View>
      <ScrollView contentContainerStyle={[styles.content, { paddingLeft: spacing.md + insets.left, paddingRight: spacing.md + insets.right, paddingBottom: Math.max(insets.bottom, spacing.xl) }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sectionHeading}><Text style={styles.sectionTitle}>Activité professionnelle</Text><Text style={styles.sectionHelp}>Touchez une carte pour démarrer une mise en relation avec un message prêt à envoyer.</Text></View>
        <View style={styles.businessGrid}>{displayedBusinessItems.map((item, index) => <Pressable key={item.id ?? `${item.title}-${index}`} onPress={() => contactAbout(item)} style={({ pressed }) => [styles.businessCard, pressed && styles.pressed]}><View style={styles.businessIcon}><Ionicons name={item.kind === "product" ? "cube-outline" : item.kind === "service" ? "briefcase-outline" : "business-outline"} size={21} color={colors.orange} /></View><Text style={styles.businessTitle}>{item.title}</Text>{item.description ? <Text style={styles.businessDescription} numberOfLines={3}>{item.description}</Text> : null}<View style={styles.cardCta}><Text style={styles.cardCtaText}>Échanger à ce sujet</Text><Ionicons name="chatbubble-ellipses-outline" size={16} color={colors.violet} /></View></Pressable>)}</View>

        <Text style={styles.sectionTitleStandalone}>Derniers Temps forts</Text>
...
const styles = StyleSheet.create({
  screen: { flex: 1 }, header: { minHeight: 58, paddingBottom: spacing.sm, flexDirection: "row", alignItems: "center" }, headerButton: { width: 48, height: 48, borderRadius: 16, alignItems: "center", justifyContent: "center" }, headerTitle: { ...typography.heading3, color: colors.text, flex: 1, textAlign: "center" }, content: { width: "100%", maxWidth: 720, alignSelf: "center" },
  identity: { paddingVertical: spacing.lg, alignItems: "center" }, onlineRow: { marginTop: 10, flexDirection: "row", alignItems: "center", gap: 6 }, onlineDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: colors.success }, offlineDot: { backgroundColor: colors.textMuted }, onlineText: { color: colors.textMuted, fontSize: 11, fontWeight: "800" }, name: { ...typography.heading2, color: colors.text, marginTop: 8, textAlign: "center" }, company: { ...typography.body, color: colors.textSecondary, marginTop: 3 }, metaRow: { marginTop: 10, flexDirection: "row", flexWrap: "wrap", justifyContent: "center", gap: 8 }, cityBadge: { minHeight: 28, paddingHorizontal: 9, borderRadius: 14, backgroundColor: colors.surfaceStrong, flexDirection: "row", alignItems: "center", gap: 6 }, cityText: { color: colors.textMuted, fontSize: 11, fontWeight: "800" },
  loadingRow: { minHeight: 42, alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 8 }, loadingText: { color: colors.textMuted, fontSize: 11 }, actions: { flexDirection: "row", gap: 8 }, action: { flex: 1, minHeight: 82, borderRadius: 20, borderWidth: 1, borderColor: colors.borderSoft, backgroundColor: colors.surface, alignItems: "center", justifyContent: "center", gap: 8 }, actionIcon: { width: 44, height: 44, borderRadius: 15, alignItems: "center", justifyContent: "center" }, actionIconPlain: { width: 44, height: 44, borderRadius: 15, backgroundColor: colors.surfaceStrong, alignItems: "center", justifyContent: "center" }, actionText: { color: colors.textSecondary, fontSize: 11, fontWeight: "900" }, audioAction: { minHeight: 52, marginTop: 8, borderRadius: 17, backgroundColor: colors.surfaceStrong, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 9 }, audioText: { color: colors.text, fontSize: 12, fontWeight: "900" },
  sectionHeading: { marginTop: spacing.xl, marginBottom: 10 }, sectionTitle: { ...typography.heading2, color: colors.text }, sectionHelp: { ...typography.bodySmall, color: colors.textMuted, marginTop: 4 }, sectionTitleStandalone: { ...typography.heading2, color: colors.text, marginTop: spacing.xl, marginBottom: 10 }, businessGrid: { flexDirection: "row", flexWrap: "wrap", gap: 10 }, businessCard: { flexGrow: 1, flexBasis: 210, minHeight: 162, padding: 14, borderRadius: 20, borderWidth: 1, borderColor: colors.borderSoft, backgroundColor: colors.surface }, businessIcon: { width: 42, height: 42, borderRadius: 14, backgroundColor: "rgba(244,177,131,0.12)", alignItems: "center", justifyContent: "center" }, businessTitle: { ...typography.heading3, color: colors.text, marginTop: 10 }, businessDescription: { ...typography.bodySmall, color: colors.textMuted, marginTop: 5, flex: 1 }, cardCta: { minHeight: 36, marginTop: 10, borderTopWidth: 1, borderTopColor: colors.borderSoft, paddingTop: 9, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }, cardCtaText: { color: colors.violet, fontSize: 11, fontWeight: "900" },
  posts: { gap: 8 }, postCard: { padding: 13, borderRadius: 18, borderWidth: 1, borderColor: colors.borderSoft, backgroundColor: colors.surface }, postTop: { flexDirection: "row", justifyContent: "space-between", gap: 8 }, kindText: { color: colors.orange, fontSize: 10, fontWeight: "900" }, postDate: { color: colors.textMuted, fontSize: 10 }, postBody: { color: colors.textSecondary, fontSize: 12, lineHeight: 18, marginTop: 7 }, emptyPosts: { minHeight: 110, borderRadius: 18, borderWidth: 1, borderColor: colors.borderSoft, backgroundColor: colors.surface, alignItems: "center", justifyContent: "center", gap: 8 }, pressed: { opacity: 0.8, transform: [{ scale: 0.993 }] },
  missing: { flex: 1, alignItems: "center", justifyContent: "center", padding: spacing.xl, gap: spacing.md }, title: { ...typography.heading2, color: colors.text }, mutedText: { ...typography.bodySmall, color: colors.textMuted, textAlign: "center" }, primaryButton: { minHeight: 48, paddingHorizontal: spacing.lg, borderRadius: 16, backgroundColor: colors.primary, alignItems: "center", justifyContent: "center" }, primaryText: { color: colors.white, fontWeight: "900" }
});