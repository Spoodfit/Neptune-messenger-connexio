...
export default function HighlightsScreen() {
  ...
  useEffect(() => { Animated.spring(overlayProgress, { toValue: selectedMemberId ? 1 : 0, useNativeDriver: true, damping: 18, stiffness: 170, mass: 0.75 }).start(); }, [overlayProgress, selectedMemberId]);
  useEffect(() => {
    if (!publishedId) return;
    publishProgress.setValue(0);
    Animated.sequence([Animated.timing(publishProgress, { toValue: 0.55, duration: 220, useNativeDriver: true }), Animated.spring(publishProgress, { toValue: 1, useNativeDriver: true, damping: 15, stiffness: 185, mass: 0.72 })]).start();
  }, [publishedId, publishProgress]);
  ...
  const renderPost = (post: HighlightPost, compact: boolean) => {
    ...
    return <Animated.View style={newlyPublished ? { opacity: publishProgress, transform: [{ translateY: publishProgress.interpolate({ inputRange: [0, 1], outputRange: [28, 0] }) }, { scale: publishProgress.interpolate({ inputRange: [0, 1], outputRange: [0.94, 1] }) }] } : undefined}><Pressable accessibilityRole="button" accessibilityLabel="Ouvrir le Temps fort" onPress={() => openPost(post.id)} style={({ pressed }) => [styles.postPressable, pressed && styles.postPressed]}><HighlightCard post={post} compact={compact} onReact={env.mockMode || BACKEND_CAPABILITIES.highlightsCommunity ? (emoji) => togglePostReaction(post.id, emoji) : undefined} /></Pressable></Animated.View>;
  };
    ...
    <LinearGradient colors={theme.pageGradient} style={styles.screen}>
      ...
      {mode === "feed" ? (
        ...
        <View style={styles.mapStage}>
          ...
          {selectedMoment ? (
            <Animated.View style={[styles.momentOverlay, { borderColor: theme.border, backgroundColor: theme.shellBackground, shadowColor: theme.shadow, opacity: overlayProgress, transform: [{ translateY: overlayProgress.interpolate({ inputRange: [0, 1], outputRange: [28, 0] }) }] }]}>
              <View style={styles.memberSummary}>
                <Pressable onPress={() => router.push(`/profile/${encodeURIComponent(selectedMoment.member.id)}`)} style={styles.memberIdentity}><StatusAvatar user={selectedMoment.member} size={43} accessible={false} /><View style={styles.memberText}><Text style={[styles.memberName, { color: theme.pageText }]} numberOfLines={1}>{selectedMoment.member.name}</Text><Text style={[styles.memberMeta, { color: theme.pageTextMuted }]} numberOfLines={1}>{selectedMoment.member.company} · position approximative</Text></View></Pressable>
                <Pressable onPress={() => setSelectedMemberId(null)} style={[styles.closeButton, { backgroundColor: theme.surfaceStrong }]}><Ionicons name="close" size={20} color={theme.pageTextMuted} /></Pressable>
              </View>
              <ScrollView style={styles.momentScroll} contentContainerStyle={styles.floatingMoments} showsVerticalScrollIndicator={false}>
                {selectedPosts.slice(0, 3).map((post, index) => <View key={post.id} style={[styles.momentBubble, { marginLeft: index * 12, marginRight: Math.max(0, 24 - index * 10), borderColor: theme.borderSoft, backgroundColor: theme.surface }]}><Pressable onPress={() => openPost(post.id)}><View style={styles.momentTop}><Text style={[styles.momentKind, { color: theme.orange }]}>{post.kind.toLocaleUpperCase("fr")}</Text><Text style={[styles.momentDate, { color: theme.pageTextMuted }]}>{new Date(post.createdAt).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}</Text></View><Text style={[styles.momentBody, { color: theme.pageTextSecondary }]} numberOfLines={3}>{post.body}</Text></Pressable><View style={styles.bubbleActions}>{(["❤️", "🔥", "👏"] as const).map((emoji) => <Pressable key={emoji} onPress={() => togglePostReaction(post.id, emoji)} style={styles.bubbleReaction}><Text style={styles.bubbleEmoji}>{emoji}</Text></Pressable>)}<Pressable onPress={() => openPost(post.id)} style={styles.bubbleComment}><Ionicons name="chatbubble-outline" size={17} color={theme.pageTextMuted} /><Text style={[styles.momentStat, { color: theme.pageTextMuted }]}>{post.comments.length}</Text></Pressable></View></View>)}
              </ScrollView>
              <View style={styles.quickActions}>{openingAction ? <View style={styles.actionLoader}><ActivityIndicator size="small" color={theme.violet} /></View> : null}{([ ["message", "chatbubble", "Message"], ["audio", "call", "Appeler"], ["video", "videocam", "Visio"] ] as const).map(([action, icon, label]) => <Pressable key={action} disabled={openingAction} onPress={() => void openSelectedAction(action)} style={[styles.quickAction, { backgroundColor: theme.surfaceStrong }]}><Ionicons name={icon} size={19} color={theme.pageText} /><Text style={[styles.quickText, { color: theme.pageText }]}>{label}</Text></Pressable>)}</View>
            </Animated.View>
          ) : <View style={[styles.mapHint, { borderColor: theme.border, backgroundColor: theme.shellBackground }]}><Ionicons name="sparkles" size={16} color={theme.orange} /><Text style={[styles.mapHintText, { color: theme.pageTextSecondary }]}>Les contours pulsants indiquent une publication récente.</Text></View>}
        </View>
      )}
    </LinearGradient>
  );
...
const styles = StyleSheet.create({
  screen: { flex: 1 }, toolbar: { width: "100%", maxWidth: 720, alignSelf: "center", paddingHorizontal: 10, paddingTop: 10, paddingBottom: 8 }, modeBar: { height: 52, padding: 3, borderRadius: 16, borderWidth: 1, flexDirection: "row", overflow: "hidden" }, modeButton: { flex: 1, minHeight: 48, overflow: "hidden", borderRadius: 13, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 }, modeLabel: { fontSize: 11, fontWeight: "900" }, postPressable: { borderRadius: 22 }, postPressed: { opacity: 0.96, transform: [{ scale: 0.996 }] }, feed: { width: "100%", maxWidth: 720, alignSelf: "center", paddingHorizontal: 10, paddingBottom: 24 }, rows: { gap: 9 }, wideRow: { width: "100%" }, pairRow: { width: "100%", flexDirection: "row", alignItems: "stretch", gap: 9 }, halfColumn: { flex: 1, minWidth: 0 }, emptyFeed: { width: "100%", minHeight: 180, alignItems: "center", justifyContent: "center", gap: 8 }, emptyText: { ...typography.bodySmall },
  mapStage: { flex: 1, width: "100%", maxWidth: 720, alignSelf: "center", paddingHorizontal: 10, paddingBottom: 12, position: "relative" }, mapHint: { position: "absolute", left: 22, right: 22, bottom: 24, minHeight: 48, paddingHorizontal: 12, borderRadius: 15, borderWidth: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 }, mapHintText: { fontSize: 14, lineHeight: 20, fontWeight: "800" }, momentOverlay: { position: "absolute", left: 18, right: 18, bottom: 20, maxHeight: "78%", padding: 10, borderRadius: 24, borderWidth: 1, shadowOpacity: 0.25, shadowRadius: 26, shadowOffset: { width: 0, height: 16 }, elevation: 10 }, memberSummary: { minHeight: 54, flexDirection: "row", alignItems: "center", gap: 8 }, memberIdentity: { flex: 1, minWidth: 0, minHeight: 50, flexDirection: "row", alignItems: "center", gap: 9 }, memberText: { flex: 1, minWidth: 0 }, memberName: { fontSize: 14, fontWeight: "900" }, memberMeta: { fontSize: 11, marginTop: 2 }, closeButton: { width: 48, height: 48, borderRadius: 15, alignItems: "center", justifyContent: "center" }, momentScroll: { maxHeight: 310, marginTop: 6 }, floatingMoments: { gap: 8, paddingBottom: 4 }, momentBubble: { padding: 11, borderRadius: 18, borderWidth: 1 }, momentTop: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8 }, momentKind: { fontSize: 11, fontWeight: "900" }, momentDate: { fontSize: 11 }, momentBody: { fontSize: 11, lineHeight: 16, marginTop: 5 }, bubbleActions: { minHeight: 48, marginTop: 5, flexDirection: "row", alignItems: "center", gap: 2 }, bubbleReaction: { width: 48, height: 48, alignItems: "center", justifyContent: "center" }, bubbleEmoji: { fontSize: 17 }, bubbleComment: { minWidth: 48, height: 48, paddingHorizontal: 8, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 }, momentStat: { fontSize: 11, fontWeight: "800" }, quickActions: { minHeight: 56, marginTop: 7, flexDirection: "row", alignItems: "center", gap: 8, position: "relative" }, quickAction: { flex: 1, minHeight: 48, borderRadius: 16, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 }, quickText: { fontSize: 11, fontWeight: "900" }, actionLoader: { position: "absolute", left: 0, right: 0, top: -38, alignItems: "center" }
});
