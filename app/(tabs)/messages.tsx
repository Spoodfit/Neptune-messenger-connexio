...
export default function MessagesScreen() {
  ...
  const sortedConversations = useMemo(() => {
    ...
    return [...byId.values()]
      ...
      .map((rawConversation) => {
        ...
        return Number.isFinite(localTimestamp) && localTimestamp > serverTimestamp ? { ...conversation, lastMessage: localLatestMessage.body, lastMessageAt: localLatestMessage.createdAt } : conversation;
      })
  ...
  if (!serviceAvailable) return (
    <LinearGradient colors={theme.pageGradient} style={styles.screen}>
      ...
      <View style={styles.feedbackWrap}><LinearGradient colors={gradients.glass} style={styles.feedback}><Ionicons name="shield-checkmark-outline" size={36} color={colors.violet} /><Text style={styles.feedbackTitle}>Messagerie temporairement protégée</Text><Text style={styles.feedbackText}>Les conversations resteront désactivées jusqu’à l’activation du backend sécurisé.</Text></LinearGradient></View>
    </LinearGradient>
  );
    ...
    <LinearGradient colors={theme.pageGradient} style={styles.screen}>
      ...
      <View style={styles.toolbar}><View accessibilityRole="tablist" style={styles.segmented}>
        {(["groups", "private"] as const).map((value) => {
          ...
          return <Pressable key={value} accessibilityRole="tab" accessibilityState={{ selected: active }} onPress={() => setFilter(value)} style={styles.segmentButton}>{active ? <LinearGradient colors={gradients.activeTab} style={StyleSheet.absoluteFill} /> : null}<Ionicons name={value === "groups" ? "people" : "chatbubble-ellipses"} size={17} color={active ? colors.text : colors.textMuted} /><Text style={[styles.segmentText, active && styles.segmentTextActive]}>{value === "groups" ? "Groupes" : "Privées"}</Text></Pressable>;
        })}
      </View></View>
      <View style={styles.sectionHeader}><Text style={[styles.sectionTitle, { color: theme.pageText }]}>{filter === "groups" ? "Discussions de groupe" : "Discussions privées"}</Text><View style={styles.sectionCountShell}><Text style={styles.sectionCount}>{sortedConversations.length}</Text></View></View>

      {lastError && sortedConversations.length === 0 ? <View style={styles.feedbackWrap}><LinearGradient colors={gradients.glass} style={styles.feedback}><Text style={styles.feedbackTitle}>Discussions indisponibles</Text><Text style={styles.feedbackText}>{lastError}</Text><Pressable onPress={() => void refreshConversations()} style={styles.retryButton}><Text style={styles.retryText}>Réessayer</Text></Pressable></LinearGradient></View> : loadingConversations && sortedConversations.length === 0 ? <View style={styles.feedbackWrap}><ActivityIndicator size="large" color={colors.violet} /></View> : (
        <FlatList
      ...
      <Modal transparent animationType="fade" visible={Boolean(selectedConversation)} onRequestClose={closeMenu}>
        <Pressable style={[styles.modalBackdrop, { backgroundColor: theme.overlay }]} onPress={closeMenu}><Pressable style={styles.sheet} onPress={() => undefined}>
          <View style={styles.sheetHandle} /><Pressable onPress={closeMenu} style={styles.closeButton}><Ionicons name="close" size={21} color={colors.textMuted} /></Pressable>
          <Text style={styles.sheetTitle}>{selectedConversation?.name}</Text><Text style={styles.sheetSubtitle}>{selectedConversation?.type === "direct" ? "Conversation privée" : `${selectedConversation?.memberIds?.length ?? selectedConversation?.memberCount ?? 0} membres`}</Text>
          <Pressable style={styles.sheetAction} onPress={toggleMute}><Ionicons name={selectedConversation?.muted ? "notifications" : "notifications-off"} size={21} color={colors.text} /><Text style={styles.sheetActionText}>{selectedConversation?.muted ? "Réactiver les notifications" : "Mettre en sourdine"}</Text></Pressable>
          <Pressable style={styles.sheetAction} onPress={openDetails}><Ionicons name="settings-outline" size={21} color={colors.text} /><Text style={styles.sheetActionText}>{selectedConversation && isPrivateConversation(selectedConversation) ? "Informations de la conversation" : "Paramètres et membres du groupe"}</Text></Pressable>
...
const styles = StyleSheet.create({
  screen: { flex: 1 }, toolbar: { width: "100%", maxWidth: MAX_CONTENT_WIDTH, alignSelf: "center", paddingHorizontal: 10, paddingTop: 10 }, segmented: { height: 52, padding: 3, borderRadius: 16, borderWidth: 1, borderColor: colors.borderSoft, backgroundColor: colors.surface, flexDirection: "row", overflow: "hidden" }, segmentButton: { flex: 1, minHeight: 48, borderRadius: 13, overflow: "hidden", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 }, segmentText: { color: colors.textMuted, fontSize: 14, fontWeight: "800" }, segmentTextActive: { color: colors.text }, sectionHeader: { width: "100%", maxWidth: MAX_CONTENT_WIDTH, alignSelf: "center", paddingHorizontal: spacing.md, paddingTop: 14, paddingBottom: 10, flexDirection: "row", alignItems: "center", gap: spacing.sm }, sectionTitle: { ...typography.heading3, flex: 1, fontWeight: "900" }, sectionCountShell: { minWidth: 28, height: 28, paddingHorizontal: 7, borderRadius: 14, borderWidth: 1, borderColor: colors.borderSoft, backgroundColor: "rgba(107,79,234,0.18)", alignItems: "center", justifyContent: "center" }, sectionCount: { color: colors.text, fontSize: 11, fontWeight: "900" }, listViewport: { flex: 1, minHeight: 0, width: "100%", maxWidth: MAX_CONTENT_WIDTH, alignSelf: "center" }, list: { paddingHorizontal: 10, paddingBottom: 22 }, emptyList: { flexGrow: 1 }, feedbackWrap: { width: "100%", maxWidth: MAX_CONTENT_WIDTH, alignSelf: "center", flex: 1, paddingHorizontal: spacing.md, alignItems: "center", justifyContent: "center" }, feedback: { width: "100%", maxWidth: 430, padding: spacing.lg, borderRadius: 24, borderWidth: 1, borderColor: colors.borderSoft, alignItems: "center", gap: spacing.sm }, feedbackTitle: { ...typography.heading3, color: colors.text, textAlign: "center" }, feedbackText: { ...typography.body, color: colors.textMuted, textAlign: "center" }, emptyState: { flex: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: spacing.lg, gap: spacing.sm }, emptyHeading: { ...typography.heading3, textAlign: "center" }, emptyText: { ...typography.body, textAlign: "center" }, retryButton: { minHeight: 48, minWidth: 124, marginTop: spacing.sm, paddingHorizontal: spacing.md, borderRadius: 16, alignItems: "center", justifyContent: "center", backgroundColor: colors.primary }, retryText: { color: colors.white, fontWeight: "900" }, modalBackdrop: { flex: 1, justifyContent: "flex-end" }, sheet: { width: "100%", maxWidth: 640, alignSelf: "center", paddingHorizontal: spacing.md, paddingTop: 10, paddingBottom: 28, borderTopLeftRadius: 28, borderTopRightRadius: 28, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surfaceStrong }, sheetHandle: { width: 42, height: 4, borderRadius: 2, alignSelf: "center", marginBottom: spacing.md, backgroundColor: colors.border }, closeButton: { position: "absolute", top: 8, right: 10, width: 48, height: 48, alignItems: "center", justifyContent: "center" }, sheetTitle: { ...typography.heading2, color: colors.text, textAlign: "center" }, sheetSubtitle: { ...typography.caption, color: colors.textMuted, textAlign: "center", marginTop: 3, marginBottom: spacing.md }, sheetAction: { minHeight: 52, borderRadius: 16, paddingHorizontal: spacing.md, flexDirection: "row", alignItems: "center", gap: 12 }, sheetActionText: { ...typography.body, color: colors.text, flex: 1, fontWeight: "700" }, dangerAction: { marginTop: 4, backgroundColor: colors.dangerSoft }, dangerText: { color: colors.danger }
});
