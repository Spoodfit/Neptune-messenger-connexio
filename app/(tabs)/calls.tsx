...
function frequentMembers(history: ReturnType<typeof useExperience>["callHistory"], members: AppUser[], currentUserId: string): AppUser[] {
  ...
  return members.filter((member) => member.id !== currentUserId).sort((a, b) => (scores.get(b.id) ?? 0) - (scores.get(a.id) ?? 0)).filter((member) => (scores.get(member.id) ?? 0) > 0).slice(0, 6);
}
...
export default function CallsScreen() {
  ...
  const searchResults = useMemo(() => {
    ...
    return members.filter((member) => member.id !== currentUser.id).filter((member) => [member.name, member.company, member.city, member.roleLabel].join(" ").toLocaleLowerCase("fr").includes(normalizedQuery)).slice(0, 12);
  }, [currentUser.id, members, normalizedQuery]);
  const upcoming = useMemo(() => scheduledCalls.filter((call) => call.status === "pending" || call.status === "accepted").filter((call) => Date.parse(call.scheduledAt) > Date.now() - 5 * 60_000).sort((a, b) => Date.parse(a.scheduledAt) - Date.parse(b.scheduledAt)), [scheduledCalls]);
  ...
  const ensureConversation = async (memberId: string) => {
    ...
    const conversation = api ? await api.createPrivateConversation([memberId]) : createPrivateConversation({ memberIds: [memberId] });
  ...
  const pageTitle = { color: theme.pageText };
  const pageMeta = { color: theme.pageTextMuted };
