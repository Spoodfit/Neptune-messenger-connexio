  ...
  return (
    <LinearGradient colors={theme.pageGradient} style={styles.screen}>
      ...
      <ScrollView
        ...
        contentContainerStyle={[styles.content, { paddingBottom: Math.max(insets.bottom, 18) }]}
      ...
      >
        ...
        <View accessibilityRole="radiogroup" accessibilityLabel={t("Type d’appel")} style={styles.modeRow}>{(["audio", "video"] as const).map((value) => { const active = mode === value; return <Pressable key={value} accessibilityRole="radio" accessibilityLabel={value === "audio" ? "Appel audio" : "Appel vidéo"} accessibilityState={{ selected: active }} onPress={() => setMode(value)} style={[styles.modeButton, { borderColor: active ? theme.violet : theme.borderSoft, backgroundColor: active ? colors.primary : theme.surface }]}><Ionicons name={value === "audio" ? "call" : "videocam"} size={20} color={active ? colors.white : theme.pageTextMuted} /><Text style={[styles.modeText, { color: active ? colors.white : theme.pageTextMuted }]}>{value === "audio" ? "Audio" : "Visio"}</Text></Pressable>; })}</View>
        <Text style={[styles.sectionTitle, heading]}>Quel jour ?</Text>
        <ScrollView horizontal accessibilityRole="radiogroup" accessibilityLabel={t("Jour du rendez-vous")} showsHorizontalScrollIndicator={false} contentContainerStyle={styles.dayRow}>{days.map((date, index) => { const key = dateKey(date); const active = key === day; const label = index === 0 ? "Aujourd’hui" : index === 1 ? "Demain" : date.toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long" }); return <Pressable key={key} accessibilityRole="radio" accessibilityLabel={label} accessibilityState={{ selected: active }} onPress={() => setDay(key)} style={[styles.dayCard, { borderColor: active ? theme.violet : theme.borderSoft, backgroundColor: active ? theme.violetSoft : theme.surface }]}><Text style={[styles.dayName, { color: active ? theme.violet : theme.pageTextMuted }]}>{index === 0 ? "Aujourd’hui" : index === 1 ? "Demain" : date.toLocaleDateString("fr-FR", { weekday: "short" })}</Text><Text style={[styles.dayNumber, { color: active ? theme.violet : theme.pageText }]}>{date.getDate()}</Text><Text style={[styles.dayMonth, { color: active ? theme.violet : theme.pageTextMuted }]}>{date.toLocaleDateString("fr-FR", { month: "short" })}</Text></Pressable>; })}</ScrollView>
        <Text style={[styles.sectionTitle, heading]}>À quelle heure ?</Text>
        <View accessibilityRole="radiogroup" accessibilityLabel={t("Heure du rendez-vous")} style={styles.timeGrid}>{TIMES.map((value) => { const active = value === time; const disabled = makeDate(day, value).getTime() <= Date.now() + 60_000; return <Pressable key={value} accessibilityRole="radio" accessibilityLabel={`${value.replace(":", " h ")}`} accessibilityState={{ selected: active, disabled }} disabled={disabled} onPress={() => setTime(value)} style={[styles.timeChip, { borderColor: active ? theme.violet : theme.borderSoft, backgroundColor: active ? colors.primary : theme.surface }, disabled && styles.disabled]}><Text style={[styles.timeText, { color: active ? colors.white : theme.pageTextSecondary }]}>{value}</Text></Pressable>; })}</View>
