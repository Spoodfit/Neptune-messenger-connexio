  ...
  return (
    <LinearGradient colors={gradients.screen} style={styles.screen}>
      ...
      >
        ...
        <View style={styles.roleBadge}><Ionicons name="shield-checkmark-outline" size={17} color={colors.success} /></View>
      </View>
      ...
      >
        <View style={styles.infoCard}>
          <Ionicons name="people-outline" size={21} color={colors.orange} />
        ...
        <View style={styles.formCard}>
          ...
          <TextInput value={automationName} onChangeText={setAutomationName} placeholder="Ex. Rappel atelier chaque lundi" placeholderTextColor={colors.textMuted} maxLength={80} style={styles.input} />

          <Text style={styles.label}>Message</Text>
          <TextInput value={body} onChangeText={setBody} placeholder="Écrivez le message qui sera envoyé…" placeholderTextColor={colors.textMuted} multiline maxLength={4000} style={[styles.input, styles.messageInput]} />
            ...
            {PRESETS.map((preset) => {
              ...
              return (
                <Pressable key={preset.id} accessibilityRole="radio" accessibilityState={{ selected }} onPress={() => applyPreset(preset)} style={[styles.presetButton, selected && styles.presetSelected]}>
                  <Ionicons name={preset.icon} size={18} color={selected ? colors.orange : colors.textMuted} />
          ...
          <View style={styles.dateRow}>
            <View style={styles.dateField}>
              ...
              <TextInput value={dateValue} onChangeText={(value) => { setDateValue(value); setSelectedPreset(""); }} keyboardType="numbers-and-punctuation" placeholder="JJ/MM/AAAA" placeholderTextColor={colors.textMuted} style={styles.input} />
            </View>
            <View style={styles.timeField}>
              <Text style={styles.label}>Heure</Text>
              <TextInput value={timeValue} onChangeText={(value) => { setTimeValue(value); setSelectedPreset(""); }} keyboardType="numbers-and-punctuation" placeholder="HH:MM" placeholderTextColor={colors.textMuted} style={styles.input} />
            </View>
          </View>
          ...
          <View style={styles.enabledRow}>
            ...
            <Switch accessibilityLabel="Activer l’automatisation" value={enabled} onValueChange={setEnabled} trackColor={{ false: colors.surfaceMuted, true: colors.primary }} thumbColor={colors.white} />
          </View>
        ...
        {loading ? (
          ...
          <View style={styles.emptyCard}>
            <Ionicons name="repeat-outline" size={30} color={colors.textMuted} />
              ...
              return (
                <View key={item.id} style={[styles.automationCard, !item.enabled && styles.pausedCard]}>
                  <View style={styles.automationTop}>
                    <View style={styles.automationIcon}><Ionicons name={item.frequency === "once" ? "send-outline" : "repeat"} size={20} color={item.enabled ? colors.orange : colors.textMuted} /></View>
                    ...
                    <Switch accessibilityLabel={`${item.enabled ? "Mettre en pause" : "Activer"} ${item.name}`} disabled={!manageable || busyId === item.id} value={item.enabled} onValueChange={() => void toggleEnabled(item)} trackColor={{ false: colors.surfaceMuted, true: colors.primary }} thumbColor={colors.white} />
                  </View>
                  ...
                  <View style={styles.cardActions}>
                    ...
                    <Pressable accessibilityRole="button" disabled={!manageable || busyId === item.id} onPress={() => deleteAutomation(item)} style={[styles.cardAction, styles.deleteAction, !manageable && styles.actionDisabled]}>
                      {busyId === item.id ? <ActivityIndicator size="small" color={colors.danger} /> : <Ionicons name="trash-outline" size={18} color={colors.danger} />}
...
const styles = StyleSheet.create({
  ...
  headerTitle: { ...typography.heading3, color: colors.text, maxWidth: "100%" },
  ...
  infoText: { flex: 1, color: colors.textSecondary, fontSize: 9.5, lineHeight: 14 },
  sectionTitle: { ...typography.heading3, color: colors.text, marginTop: spacing.lg, marginBottom: 9 },
  formCard: { padding: spacing.md, borderRadius: radii.xl, borderWidth: 1, borderColor: colors.borderSoft, backgroundColor: colors.surface },
  label: { color: colors.textSecondary, fontSize: 10, fontWeight: "900", marginBottom: 6, marginTop: 10 },
  input: { minHeight: 48, paddingHorizontal: 13, paddingVertical: 11, borderRadius: 16, borderWidth: 1, borderColor: colors.borderSoft, backgroundColor: colors.surfaceStrong, color: colors.text, fontSize: 12 },
  messageInput: { minHeight: 110, textAlignVertical: "top" },
  counter: { color: colors.textMuted, fontSize: 8.5, textAlign: "right", marginTop: 4 },
  frequencyGrid: { flexDirection: "row", flexWrap: "wrap", gap: 7 },
  frequencyButton: { flexGrow: 1, flexBasis: 135, minHeight: 58, padding: 10, borderRadius: 16, borderWidth: 1, borderColor: colors.borderSoft, backgroundColor: colors.surfaceStrong },
  frequencySelected: { borderColor: colors.violet, backgroundColor: "rgba(107,79,234,0.18)" },
  frequencyLabel: { color: colors.textMuted, fontSize: 10.5, fontWeight: "900" },
  frequencyLabelSelected: { color: colors.text },
  frequencyDescription: { color: colors.textMuted, fontSize: 8.5, marginTop: 3 },
  presetGrid: { flexDirection: "row", flexWrap: "wrap", gap: 7 },
  presetButton: { flexGrow: 1, flexBasis: 130, minHeight: 48, paddingHorizontal: 10, borderRadius: 15, borderWidth: 1, borderColor: colors.borderSoft, backgroundColor: colors.surfaceStrong, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6 },
  presetSelected: { borderColor: colors.orange, backgroundColor: "rgba(244,177,131,0.11)" },
  presetText: { color: colors.textMuted, fontSize: 9.5, fontWeight: "800" },
  presetTextSelected: { color: colors.orange },
  ...
  enabledTitle: { color: colors.text, fontSize: 11, fontWeight: "900" },
  enabledSubtitle: { color: colors.textMuted, fontSize: 9, marginTop: 3 },
  formActions: { marginTop: spacing.md, flexDirection: "row", gap: 8 },
  cancelButton: { minHeight: 50, paddingHorizontal: 16, borderRadius: 16, borderWidth: 1, borderColor: colors.borderSoft, alignItems: "center", justifyContent: "center" },
  cancelText: { color: colors.textSecondary, fontSize: 11, fontWeight: "900" },
  saveAutomationButton: { flex: 1, minHeight: 50, paddingHorizontal: 16, borderRadius: 16, backgroundColor: colors.primary, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7 },
  saveAutomationText: { color: colors.white, fontSize: 11, fontWeight: "900" },
  listHeader: { flexDirection: "row", alignItems: "center", gap: 8 },
  countBadge: { minWidth: 26, height: 26, paddingHorizontal: 7, borderRadius: 13, backgroundColor: colors.surfaceStrong, alignItems: "center", justifyContent: "center", marginTop: spacing.lg, marginBottom: 9 },
  countText: { color: colors.textSecondary, fontSize: 9, fontWeight: "900" },
  loading: { minHeight: 100, alignItems: "center", justifyContent: "center" },
  emptyCard: { minHeight: 150, padding: spacing.lg, borderRadius: radii.xl, borderWidth: 1, borderColor: colors.borderSoft, backgroundColor: colors.surface, alignItems: "center", justifyContent: "center" },
  emptyTitle: { color: colors.text, fontSize: 13, fontWeight: "900", marginTop: 9 },
  emptyText: { color: colors.textMuted, fontSize: 9.5, marginTop: 4, textAlign: "center" },
  automationList: { gap: 9 },
  automationCard: { padding: 13, borderRadius: radii.xl, borderWidth: 1, borderColor: colors.borderSoft, backgroundColor: colors.surface },
  pausedCard: { opacity: 0.7 },
  automationTop: { flexDirection: "row", alignItems: "center", gap: 10 },
  automationIcon: { width: 42, height: 42, borderRadius: 14, backgroundColor: colors.surfaceStrong, alignItems: "center", justifyContent: "center" },
  automationContent: { flex: 1, minWidth: 0 },
  automationName: { color: colors.text, fontSize: 12, lineHeight: 16, fontWeight: "900" },
  automationMeta: { color: colors.textMuted, fontSize: 8.5, marginTop: 3 },
  creator: { color: colors.orange, fontSize: 8.5, marginTop: 3, fontWeight: "800" },
  automationBody: { color: colors.textSecondary, fontSize: 10, lineHeight: 15, marginTop: 10 },
  cardActions: { marginTop: 10, flexDirection: "row", gap: 7 },
  cardAction: { flex: 1, minHeight: 44, borderRadius: 14, backgroundColor: colors.surfaceStrong, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6 },
  deleteAction: { backgroundColor: colors.dangerSoft },
  cardActionText: { color: colors.textSecondary, fontSize: 9, fontWeight: "900" },
  deleteText: { color: colors.danger, fontSize: 9, fontWeight: "900" },
  actionDisabled: { opacity: 0.35 },
  readOnlyText: { color: colors.textMuted, fontSize: 8, textAlign: "center", marginTop: 7 },
  ...
  description: { ...typography.body, color: colors.textMuted, textAlign: "center", maxWidth: 460 },
  secondaryButton: { minHeight: 48, paddingHorizontal: spacing.lg, borderRadius: 16, borderWidth: 1, borderColor: colors.borderSoft, backgroundColor: colors.surface, alignItems: "center", justifyContent: "center" },
