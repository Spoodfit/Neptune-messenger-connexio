...
function migrateChat() {
  ...
  source = source.split("colors={gradients.activeTab}").join("colors={theme.isLight ? [theme.accentSoft, theme.violetSoft] : gradients.activeTab}");
  source = semanticize(source);
  fs.writeFileSync(path, source);
}
