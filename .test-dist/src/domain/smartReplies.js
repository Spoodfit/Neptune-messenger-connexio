...
exports.buildSmartReplySuggestions = buildSmartReplySuggestions;
const normalize = (value) => value.toLocaleLowerCase("fr").normalize("NFD").replace(/[\u0300-\u036f]/g, "");
const unique = (items) => [...new Set(items)].slice(0, 3);
function buildSmartReplySuggestions(message, currentUser) {
    if (!message || message.isMine || !message.body.trim())
        return [];
    ...
    const mentioned = message.mentionedUserIds?.includes(currentUser.id) ||
        mentionTokens.some((token) => body.includes(token));
    const replies = [];
    if (mentioned) {
        replies.push("Je viens de voir ta mention — je m’en occupe.", "Bien reçu, je te réponds juste après.", "Tu as besoin de quoi précisément ?");
    }
    if (/\?|qui |quoi |quand |comment |pourquoi |est-ce|peux-tu|pouvez-vous/.test(body)) {
        replies.push("Oui, je regarde ça.", "Je te réponds dans quelques minutes.", "Peux-tu me donner un peu plus de détails ?");
    }
    if (/rdv|rendez-vous|creneau|dispo|disponible|agenda|heure|demain|vendredi|lundi/.test(body)) {
        replies.push("Oui, c’est bon pour moi.", "Je vérifie mon agenda.", "Propose-moi deux créneaux.");
    }
