...
function containsInsult(normalized: string): boolean {
  return INSULTS.some((word) =>
    new RegExp(`(^|[^a-z])${normalizeText(word)}([^a-z]|$)`, "u").test(normalized)
  );
}

function containsForcedCommercialPressure(normalized: string): boolean {
...
function looksLikeSpam(body: string, normalized: string): boolean {
...
function isRepeatedAdvertising(
  normalized: string,
  recentBodies: readonly string[]
): boolean {
  ...
  const commercialSignal =
    /\b(prix|promo|offre|devis|acheter|commander|reservation|réservation|lien)\b/u.test(
      normalized
    ) || countLinks(normalized) > 0;
  return commercialSignal && duplicates >= 2;
}

function penaltyForWarning(
  warningCount: number,
  now: Date
): Pick<
  ModerationDecision,
  "warningLevel" | "suspendedUntil" | "requiresManualReview"
> {
  const nextWarning = Math.min(3, Math.max(1, warningCount + 1)) as 1 | 2 | 3;
  if (nextWarning === 1) return { warningLevel: 1 };
  if (nextWarning === 2) {
    return {
      warningLevel: 2,
...
export function evaluateModeration(input: ModerationInput): ModerationDecision {
  ...
  if (containsInsult(normalized)) {
    return blockedDecision(
      "insult",
      "Le message contient une insulte ou une formulation dégradante.",
      input
    );
  }
  if (containsForcedCommercialPressure(normalized)) {
    return blockedDecision(
      "forced_commercial",
      "La sollicitation commerciale exerce une pression excessive sur le destinataire.",
      input
    );
  }
  if (isRepeatedAdvertising(normalized, input.recentBodies ?? [])) {
    return blockedDecision(
      ...
      "Une publicité identique a déjà été envoyée plusieurs fois récemment.",
      input
    );
  }
  if (looksLikeSpam(input.body, normalized)) {
    return blockedDecision(
      "spam",
      "Le message présente des caractéristiques de spam ou de publication automatisée abusive.",
      input
    );
  }
  return { allowed: true };
