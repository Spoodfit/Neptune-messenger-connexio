...
type MarkerSelection = { marker: CoworkingMapMarker; member: AppUser; space?: CoworkingSpace };
...
export default function UnifiedMapScreenV25() {
  const insets = useSafeAreaInsets();
  const theme = useAppTheme();
  const { localeTag } = useAppLanguage();
  const { currentUser, accessToken } = useSession();
  const { members, mapMoments } = useExperience();
  const { serviceAvailable, snapshot, loading, refresh, joinSpace, createSpace } = useCoworking();
  ...
  const [mapMedia, setMapMedia] = useState<CoworkingMediaSession>();
  ...
  useEffect(() => { void loadEvents(); }, [eventsApi]);
  ...
  useEffect(() => {
    ...
    void mapApi.enterMap()
      ...
      .catch(() => { if (active) setMapCameraActive(false); });
    return () => {
      active = false;
  ...
  const momentByUserId = useMemo(() => new Map(mapMoments.map((moment) => [moment.member.id, moment])), [mapMoments]);
  const presenceByUserId = useMemo(() => new Map(snapshot.participants.map((presence) => [presence.userId, presence])), [snapshot.participants]);
  ...
  const markers = useMemo<CoworkingMapMarker[]>(() => {
    ...
    const result: CoworkingMapMarker[] = singles.map((member) => {
  ...
  const eventMarkers = useMemo<CoworkingMapEventMarker[]>(() => visibleEvents.map((event) => ({
    id: event.id,
    title: event.title,
  ...
  const selectedMember = selectedMarker
    ...
    : undefined;
  const selectedSpace = selectedMarker?.spaceId ? snapshot.spaces.find((space) => space.id === selectedMarker.spaceId) : undefined;
  const selection: MarkerSelection | null = selectedMarker && selectedMember ? { marker: selectedMarker, member: selectedMember, space: selectedSpace } : null;
  const selectedEvent = selectedEventId ? visibleEvents.find((event) => event.id === selectedEventId) : undefined;

  const selectMarker = (markerId: string, memberId?: string) => {
    const marker = markers.find((item) => item.id === markerId);
    setSelectedEventId(null);
    setSelectedMarkerId(markerId);
    setSelectedMemberId(memberId ?? marker?.members[0]?.id ?? null);
  };
  ...
  const knock = async () => {
    ...
    try {
      ...
      if (result.status === "accepted") {
        await enterSpace(result.spaceId ?? selection.space.id);
        return;
      }
      setNotice(`Tu as toqué chez ${firstName(selection.member.name)} · autorisation demandée.`);
    } catch (error) {
    ...
    <View style={[styles.screen, { backgroundColor: theme.pageBackground }]}>
      <CoworkingGeographicMap
        ...
        onMediaUnavailable={(message) => {
          setMapCameraActive(false);
          AppAlert.alert("Caméra indisponible", message);
        }}
      />
