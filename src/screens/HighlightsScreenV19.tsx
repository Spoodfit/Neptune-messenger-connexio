...
export default function HighlightsScreenV19() {
  ...
  const mapEvents = useMemo(() => {
    if (mapFilter === "person") return [];
    return visibleDiscoveryEvents(events, eventWindow);
  }, [eventWindow, events, mapFilter]);
  const mapPeople = mapFilter === "event" ? [] : mapMoments;

  const selectedMoment = selectedEntity?.kind === "person"
    ? mapMoments.find((moment) => moment.member.id === selectedEntity.id)
    : undefined;
  const selectedEvent = selectedEntity?.kind === "event"
    ? events.find((event) => event.id === selectedEntity.id)
    : undefined;
  const selectedPost = selectedMoment
    ? posts.find((post) => selectedMoment.recentPostIds.includes(post.id))
    : undefined;
  ...
  const ensureSelectedConversation = async () => {
    ...
    const existing = [...visibleConversations, ...localConversations].find(
      (conversation) => conversation.type === "direct" && conversation.memberIds?.includes(selectedMoment.member.id)
    );
  ...
  const eventStateLabel = (event: DiscoveryEvent) => {
    const state = getDiscoveryEventState(event);
    if (state === "live") return "En cours maintenant";
    if (state === "past24h") return "Terminé il y a moins de 24 h";
    return "À venir";
  };

  const eventStateColor = (event: DiscoveryEvent) => {
    const state = getDiscoveryEventState(event);
    if (state === "live") return theme.success;
    if (state === "past24h") return theme.warning;
    return theme.accent;
  };
    ...
    <LinearGradient colors={theme.pageGradient} style={styles.screen}>
      ...
      {mode === "feed" ? <ScrollView contentContainerStyle={styles.feedContent} showsVerticalScrollIndicator={false}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.mapFilters}>
          {([ ["all", "Tout", "apps-outline"], ["person", "Personnes", "people-outline"], ["event", "Évènements", "calendar-outline"] ] as const).map(([value, label, icon]) => {
        ...
        {mapFilter !== "person" ? <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.eventWindowRow}>
          {EVENT_WINDOWS.map((item) => {
          ...
          {eventsLoading ? <ActivityIndicator size="small" color={theme.accent} style={styles.eventLoader} /> : null}
        </ScrollView> : null}
        ...
        <View style={styles.mapStage}>
          ...
          {selectedMoment ? <View style={[styles.entitySheet, { backgroundColor: theme.shellBackground, borderColor: theme.border }]}>
            <View style={styles.sheetTop}>
              <Pressable onPress={() => router.push(`/profile/${encodeURIComponent(selectedMoment.member.id)}`)} style={styles.personIdentity}>
                <StatusAvatar user={selectedMoment.member} size={50} accessible={false} />
                <View style={styles.identityCopy}><Text style={[styles.entityTitle, { color: theme.pageText }]} numberOfLines={1}>{selectedMoment.member.name}</Text><Text style={[styles.entityMeta, { color: theme.pageTextMuted }]} numberOfLines={1}>{selectedMoment.member.company} · {selectedMoment.member.city}</Text></View>
              </Pressable>
            ...
            <View style={styles.sheetActions}>
              <Pressable disabled={openingAction} onPress={() => void openPersonAction("message")} style={[styles.primaryAction, { backgroundColor: theme.accent }]}><Ionicons name="chatbubble-ellipses" size={19} color="#fff" /><Text style={styles.primaryActionText}>Message</Text></Pressable>
              <Pressable disabled={openingAction} onPress={() => void openPersonAction("audio")} style={[styles.secondaryAction, { backgroundColor: theme.surfaceStrong }]}><Ionicons name="call-outline" size={19} color={theme.pageText} /></Pressable>
              <Pressable onPress={() => router.push(`/profile/${encodeURIComponent(selectedMoment.member.id)}`)} style={[styles.secondaryAction, { backgroundColor: theme.surfaceStrong }]}><Ionicons name="person-outline" size={19} color={theme.pageText} /></Pressable>
            </View>
          </View> : null}

          {selectedEvent ? <View style={[styles.entitySheet, { backgroundColor: theme.shellBackground, borderColor: theme.border }]}>
            <View style={styles.sheetTop}>
              <View style={[styles.eventIconLarge, { backgroundColor: eventStateColor(selectedEvent) }]}><Ionicons name="calendar" size={23} color="#fff" /></View>
              <View style={styles.identityCopy}><View style={styles.eventStateRow}><View style={[styles.stateDot, { backgroundColor: eventStateColor(selectedEvent) }]} /><Text style={[styles.eventState, { color: eventStateColor(selectedEvent) }]}>{eventStateLabel(selectedEvent)}</Text></View><Text style={[styles.entityTitle, { color: theme.pageText }]} numberOfLines={2}>{selectedEvent.title}</Text></View>
              <Pressable accessibilityLabel="Fermer" onPress={() => setSelectedEntity(null)} style={[styles.closeButton, { backgroundColor: theme.surfaceStrong }]}><Ionicons name="close" size={20} color={theme.pageTextMuted} /></Pressable>
            </View>
            <View style={styles.eventDetails}>
              <View style={styles.detailRow}><Ionicons name="time-outline" size={17} color={theme.pageTextMuted} /><Text style={[styles.detailText, { color: theme.pageTextSecondary }]}>{new Date(selectedEvent.startsAt).toLocaleString(localeTag, { weekday: "short", day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</Text></View>
              {selectedEvent.address || selectedEvent.city ? <View style={styles.detailRow}><Ionicons name="location-outline" size={17} color={theme.pageTextMuted} /><Text style={[styles.detailText, { color: theme.pageTextSecondary }]} numberOfLines={1}>{selectedEvent.address ?? selectedEvent.city}</Text></View> : null}
            </View>
...
const styles = StyleSheet.create({
  ...
  modeBar: { height: 54, padding: 3, borderRadius: 18, borderWidth: 1, flexDirection: "row", overflow: "hidden" },
  modeButton: { flex: 1, height: 46, borderRadius: 15, overflow: "hidden", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 },
  ...
  composerCard: { marginTop: 12, minHeight: 82, padding: 12, borderRadius: 22, borderWidth: 1, flexDirection: "row", alignItems: "center", gap: 10 },
  composerCopy: { flex: 1, minWidth: 0 },
  composerPrompt: { fontSize: 14, lineHeight: 19, fontWeight: "900" },
  composerHint: { fontSize: 11, lineHeight: 16, marginTop: 3 },
  composerAdd: { width: 44, height: 44, borderRadius: 15, alignItems: "center", justifyContent: "center" },
  ...
  filterChip: { minHeight: 46, paddingHorizontal: 12, borderRadius: 16, borderWidth: 1, flexDirection: "row", alignItems: "center", gap: 7 },
  filterText: { fontSize: 12, fontWeight: "850" },
  ...
  emptyText: { ...typography.bodySmall, textAlign: "center", marginTop: 5 },
  mapContent: { flex: 1, width: "100%", maxWidth: 720, alignSelf: "center", paddingHorizontal: 12, paddingBottom: 12 },
  mapFilters: { gap: 7, paddingTop: 10, paddingBottom: 7 },
  mapFilterChip: { minHeight: 44, paddingHorizontal: 12, borderRadius: 16, borderWidth: 1, flexDirection: "row", alignItems: "center", gap: 7 },
  eventWindowRow: { minHeight: 44, alignItems: "center", gap: 3, paddingBottom: 7 },
  windowChip: { minHeight: 38, paddingHorizontal: 10, borderRadius: 14, borderWidth: 1, alignItems: "center", justifyContent: "center" },
  windowText: { fontSize: 11, fontWeight: "850" },
  eventLoader: { marginLeft: 6 },
  ...
  personLegend: { width: 18, height: 18, borderRadius: 7, borderWidth: 3 },
  eventLegend: { width: 16, height: 16, borderRadius: 5, transform: [{ rotate: "45deg" }] },
  legendText: { fontSize: 11, fontWeight: "800" },
  syncWarning: { flexBasis: "100%", fontSize: 10, lineHeight: 14, paddingBottom: 4 },
  ...
  sheetTop: { minHeight: 54, flexDirection: "row", alignItems: "center", gap: 10 },
  personIdentity: { flex: 1, minWidth: 0, flexDirection: "row", alignItems: "center", gap: 10 },
  identityCopy: { flex: 1, minWidth: 0 },
  entityTitle: { fontSize: 15, lineHeight: 20, fontWeight: "900" },
  entityMeta: { fontSize: 11, marginTop: 3 },
  closeButton: { width: 48, height: 48, borderRadius: 16, alignItems: "center", justifyContent: "center" },
  latestMoment: { marginTop: 9, padding: 10, borderRadius: 17, borderWidth: 1 },
  momentEyebrow: { fontSize: 10, fontWeight: "900", letterSpacing: 0.5 },
  momentText: { fontSize: 12, lineHeight: 17, marginTop: 4 },
  sheetActions: { flexDirection: "row", gap: 8, marginTop: 10 },
  primaryAction: { flex: 1, minHeight: 48, borderRadius: 16, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7 },
  primaryActionText: { color: "#fff", fontSize: 13, fontWeight: "900" },
  secondaryAction: { width: 48, height: 48, borderRadius: 16, alignItems: "center", justifyContent: "center" },
  eventIconLarge: { width: 50, height: 50, borderRadius: 17, alignItems: "center", justifyContent: "center" },
  eventStateRow: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 3 },
  stateDot: { width: 7, height: 7, borderRadius: 4 },
  eventState: { fontSize: 10, fontWeight: "900" },
  eventDetails: { gap: 6, marginTop: 9 },
  detailRow: { minHeight: 24, flexDirection: "row", alignItems: "center", gap: 7 },
  detailText: { flex: 1, fontSize: 12, fontWeight: "750" },
  eventSummary: { fontSize: 11, lineHeight: 16, marginTop: 6 },
  eventCta: { minHeight: 48, borderRadius: 16, marginTop: 10, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 }
});
