...
export default function CoworkingMediaSurface({
  ...
  const html = useMemo(
    () => buildCoworkingMediaHtml(session, displayName, { cameraOn, microphoneOn }),
    [displayName, session]
  );

  useEffect(() => {
    webViewRef.current?.injectJavaScript(
      `window.__connexioCoworkingControl?.(${JSON.stringify({ type: "media", cameraOn, microphoneOn })});true;`
    );
  ...
  const handleMessage = (event: WebViewMessageEvent) => {
    try {
      ...
      if (payload.type === "connected" || payload.type === "media-ready") onConnected?.();
      if (payload.type === "error") onError?.(payload.message ?? "Connexion média impossible.");
    } catch {
