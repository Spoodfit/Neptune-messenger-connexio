...
export function NeptuneTabBar({ state, descriptors, navigation }: NeptuneTabBarProps) {
  ...
  const visibleRoutes = useMemo(() => state.routes.filter((route) => route.name in ICONS && (route.name !== "calls" || CALLS_AVAILABLE)), [state.routes]);
  const currentKey = state.routes[state.index]?.key;
  const activeIndex = Math.max(0, visibleRoutes.findIndex((route) => route.key === currentKey));
  const itemWidth = visibleRoutes.length ? Math.max(0, barWidth - 10) / visibleRoutes.length : 0;
  ...
  useEffect(() => {
    ...
    Animated.spring(translateX, { toValue: target, useNativeDriver: true, damping: 18, stiffness: 175, mass: 0.72 }).start();
  }, [activeIndex, itemWidth, reducedMotion, translateX]);
  ...
  return (
    <View pointerEvents="box-none" style={[styles.outer, { paddingBottom: Math.max(insets.bottom, 8) }]}>
      <View onLayout={onLayout} style={styles.bar}>
        {itemWidth > 0 ? (
          <Animated.View pointerEvents="none" style={[styles.indicator, { width: itemWidth, transform: [{ translateX }] }]}>
            <LinearGradient colors={gradients.activeTab} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.indicatorGradient} />
          </Animated.View>
        ) : null}
        {visibleRoutes.map((route) => {
          ...
          return (
            <Pressable
              ...
              style={({ pressed }) => [styles.item, { width: itemWidth || `${100 / visibleRoutes.length}%` }, pressed && styles.pressed, pressed && !reducedMotion && styles.pressedMotion]}
            >
              <View style={styles.iconWrap}>
                <Ionicons name={focused ? icon.active : icon.inactive} size={21} color={focused ? colors.text : "#7F8DAB"} />
                {badge !== undefined ? <View style={styles.badge} accessibilityElementsHidden><Text style={styles.badgeText}>{String(badge)}</Text></View> : null}
              </View>
...
const styles = StyleSheet.create({
  outer: { paddingHorizontal: 10, paddingTop: 4, backgroundColor: colors.background },
  bar: { height: 72, padding: 5, overflow: "hidden", position: "relative", borderRadius: radii.xl, borderWidth: 1, borderColor: colors.border, backgroundColor: "rgba(8,18,38,0.98)", flexDirection: "row", elevation: 18 },
  indicator: { position: "absolute", left: 5, top: 5, bottom: 5, paddingHorizontal: 2 },
  indicatorGradient: { flex: 1, borderRadius: 19, borderWidth: 1, borderColor: colors.borderSoft },
  item: { minHeight: 62, zIndex: 2, alignItems: "center", justifyContent: "center", gap: 8 },
  pressed: { opacity: 0.82 },
  pressedMotion: { transform: [{ scale: 0.96 }] },
  iconWrap: { position: "relative", width: 28, alignItems: "center" },
  label: { maxWidth: "100%", color: "#7F8DAB", fontSize: 11, lineHeight: 12, fontWeight: "800" },
