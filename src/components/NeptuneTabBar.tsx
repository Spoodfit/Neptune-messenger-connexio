...
export function NeptuneTabBar({ state, descriptors, navigation }: NeptuneTabBarProps) {
  ...
  const openNewHighlight = () => {
    setMenuOpen(false);
    const composeNonce = String(Date.now());
    requestAnimationFrame(() => navigation.navigate("highlights", { compose: "1", composeNonce }));
  };
  ...
  return (
    <View pointerEvents="box-none" style={[styles.outer, { paddingBottom: Math.max(insets.bottom, 8), backgroundColor: theme.pageBackground }]}>
      ...
      <Animated.View pointerEvents="box-none" style={[styles.createShell, { backgroundColor: theme.pageBackground, transform: [{ rotate: menuProgress.interpolate({ inputRange: [0, 1], outputRange: ["0deg", "45deg"] }) }, { scale: menuProgress.interpolate({ inputRange: [0, 1], outputRange: [1, 1.06] }) }] }]}>
        <Pressable accessibilityRole="button" accessibilityLabel={menuOpen ? "Fermer la création" : "Créer"} accessibilityState={{ expanded: menuOpen }} hitSlop={6} onPress={() => setMenuOpen((value) => !value)} style={({ pressed }) => [styles.createPressable, pressed && styles.createPressed]}>
