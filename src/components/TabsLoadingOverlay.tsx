...
export function TabsLoadingOverlay() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      void refreshExperience().finally(() => {
  ...
  return (
    <View
      pointerEvents="none"
      accessibilityLabel={feedLoading ? "Chargement des Temps forts" : "Chargement des discussions"}
  ...
  return (
    <View style={styles.list}>
      {Array.from({ length: 6 }, (_, index) => (
        <View key={`conversation-loading-${index}`} style={styles.row}>
  ...
  return (
    <View style={styles.list}>
      {Array.from({ length: 3 }, (_, index) => (
        <View key={`feed-loading-${index}`} style={styles.card}>
