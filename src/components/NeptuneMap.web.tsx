...
export default function NeptuneMap({ moments, selectedMemberId, onSelectMember }: NeptuneMapProps) {
  ...
  useEffect(() => {
    iframeRef.current?.contentWindow?.postMessage({ source: "connexio-map-parent", type: "selected-member", memberId: selectedMemberId }, window.location.origin);
  }, [selectedMemberId]);
  ...
  const html = useMemo(() => {
    ...
    return `<!doctype html>
<html lang="${uiLanguage}"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no" />
${leafletSecurityMeta()}
${LEAFLET_STYLESHEETS}
<style>
...
const parentOrigin=(()=>{try{const value=window.parent.location.origin;return value&&value!=='null'?value:null}catch{return null}})();const postToParent=payload=>{if(parentOrigin)window.parent.postMessage(payload,parentOrigin)};const members=${serialized};const markerNodes=new Map();const escapeText=value=>String(value).replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[char]));const map=L.map('map',{zoomControl:true,minZoom:5,maxZoom:16,zoomSnap:.25});L.tileLayer('https://{s}.basemaps.cartocdn.com/${tileStyle}/{z}/{x}/{y}{r}.png',{subdomains:'abcd',maxZoom:20,attribution:'&copy; OpenStreetMap &copy; CARTO'}).addTo(map);const cluster=L.markerClusterGroup({maxClusterRadius:56,spiderfyOnMaxZoom:true,showCoverageOnHover:false,iconCreateFunction:c=>L.divIcon({className:'custom-cluster',html:'<div class="cluster-core">'+c.getChildCount()+'</div>',iconSize:[48,48]})});const bounds=[];members.forEach(member=>{const avatarMarkup=member.avatarUrl?'<img src="'+escapeText(member.avatarUrl)+'" alt="" />':escapeText(member.initials);const html='<div class="member-marker '+(member.pulse?'pulse':'')+'" data-member-id="'+escapeText(member.id)+'" style="--role-color:'+escapeText(member.roleColor)+'"><div class="member-core"><div class="member-inner">'+avatarMarkup+'</div></div></div>';const marker=L.marker([member.latitude,member.longitude],{icon:L.divIcon({className:'',html,iconSize:[58,58],iconAnchor:[29,29]}),title:member.name});marker.on('add',()=>{const node=marker.getElement()?.querySelector('.member-marker');if(node)markerNodes.set(member.id,node)});marker.on('click',()=>postToParent({source:'connexio-map',type:'member-selected',memberId:member.id}));marker.bindTooltip(escapeText(member.name),{direction:'bottom',offset:[0,23],opacity:.96});cluster.addLayer(marker);bounds.push([member.latitude,member.longitude]);});map.addLayer(cluster);if(bounds.length){map.fitBounds(bounds,{padding:[54,54],maxZoom:9})}else{map.setView([43.45,2.6],7)}const updateSelection=memberId=>markerNodes.forEach((node,id)=>node.classList.toggle('selected',Boolean(memberId)&&id===memberId));window.addEventListener('message',event=>{if(event.origin!==parentOrigin)return;const data=event.data;if(data?.source==='connexio-map-parent'&&data?.type==='selected-member')updateSelection(data.memberId||null)});document.querySelector('.geo-btn').addEventListener('click',()=>{if(!navigator.geolocation)return;navigator.geolocation.getCurrentPosition(position=>{const point=[position.coords.latitude,position.coords.longitude];map.flyTo(point,13,{duration:.8});L.circle(point,{radius:Math.max(180,position.coords.accuracy),color:'${theme.orange}',fillColor:'${theme.accent}',fillOpacity:.12,weight:2}).addTo(map);},()=>{}, {enableHighAccuracy:false,timeout:8000,maximumAge:60000});});
  ...
  return <View style={[styles.wrap, { backgroundColor: theme.pageBackground, borderColor: theme.border }]}>{createElement("iframe", { ref: (node: HTMLIFrameElement | null) => { iframeRef.current = node; }, title: "Carte Neptune", srcDoc: html, onLoad: () => iframeRef.current?.contentWindow?.postMessage({ source: "connexio-map-parent", type: "selected-member", memberId: selectedMemberId }, window.location.origin), sandbox: "allow-scripts allow-same-origin", referrerPolicy: "no-referrer", style: { width: "100%", height: "100%", border: 0, display: "block", background: theme.pageBackground }, allow: "geolocation" })}</View>;
}
