      ...
      <Animated.View {...panResponder.panHandlers} style={[styles.foreground, { transform: [{ translateX }] }]}>
        <Pressable onPress={() => { close(); onOpen(); }} style={({ pressed }) => [styles.row, pressed && styles.pressed]}>
          ...
          <View style={styles.content}>
            <View style={styles.nameLine}><Text numberOfLines={1} style={styles.name}>{member.name}</Text>{isResponsible ? <View style={styles.responsibleBadge}><Ionicons name="shield-checkmark" size={12} color={colors.orange} /><Text style={styles.responsibleText}>Responsable</Text></View> : null}</View>
...
const styles = StyleSheet.create({
  stage: { minHeight: 82, overflow: "hidden", backgroundColor: colors.surfaceStrong }, divider: { borderBottomWidth: 1, borderBottomColor: colors.borderSoft }, actions: { position: "absolute", top: 0, right: 0, bottom: 0, width: ACTION_WIDTH, flexDirection: "row" }, action: { width: ACTION_WIDTH / 2, alignItems: "center", justifyContent: "center", gap: 7 }, responsibleAction: { backgroundColor: colors.primaryDark }, removeAction: { backgroundColor: colors.danger }, disabledAction: { backgroundColor: colors.surfaceMuted }, actionText: { color: colors.white, fontSize: 10, fontWeight: "900" }, foreground: { backgroundColor: colors.surface }, row: { minHeight: 82, paddingHorizontal: 12, paddingVertical: 9, flexDirection: "row", alignItems: "center", gap: 11 }, pressed: { opacity: 0.78 }, content: { flex: 1, minWidth: 0 }, nameLine: { flexDirection: "row", alignItems: "center", gap: 8, minWidth: 0 }, name: { flex: 1, minWidth: 0, color: colors.text, fontSize: 14, fontWeight: "900" }, company: { color: colors.textMuted, fontSize: 11, marginTop: 3 }, metaLine: { minHeight: 25, marginTop: 5, flexDirection: "row", alignItems: "center", gap: 8 }, responsibleBadge: { minHeight: 22, paddingHorizontal: 7, borderRadius: 11, backgroundColor: "rgba(244,177,131,0.12)", flexDirection: "row", alignItems: "center", gap: 5 }, responsibleText: { color: colors.orange, fontSize: 10, fontWeight: "900" }, swipeHint: { flex: 1, color: colors.textMuted, fontSize: 9, textAlign: "right" }, manageButton: { minWidth: 54, minHeight: 48, paddingHorizontal: 6, borderRadius: 14, alignItems: "center", justifyContent: "center", gap: 2, backgroundColor: colors.surfaceStrong }, manageText: { color: colors.textMuted, fontSize: 9, fontWeight: "900" }
});