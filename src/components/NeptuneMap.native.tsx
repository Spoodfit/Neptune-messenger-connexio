...
.member-marker{width:58px;height:58px;position:relative;display:grid;place-items:center;will-change:transform}.member-core{width:48px;height:48px;border-radius:18px;padding:2px;border:3px solid var(--role-color);background:var(--role-background);box-shadow:0 0 22px rgba(107,79,234,.45);position:relative;z-index:2;transition:transform .18s ease,box-shadow .18s ease}.member-inner{height:100%;border-radius:16px;border:2px solid #081226;background:#101a31;color:#fff;display:grid;place-items:center;font-weight:900;font-size:11px;overflow:hidden}.member-inner img{width:100%;height:100%;object-fit:cover}.member-marker.pulse:before,.member-marker.pulse:after{content:"";position:absolute;inset:3px;border-radius:50%;border:2px solid rgba(0,114,255,.55);animation:pulse 4.8s ease-out infinite}.member-marker.pulse:after{inset:-4px;border-color:rgba(244,177,131,.38);animation-delay:.2s}.member-marker.selected .member-core{transform:scale(1.12);box-shadow:0 0 0 7px rgba(0,72,186,.14),0 0 34px rgba(107,79,234,.65)}@keyframes pulse{0%,68%,100%{opacity:0;transform:scale(.84)}72%{opacity:.9}88%{opacity:0;transform:scale(1.38)}}
...
members.forEach(member=>{
 ...
 marker.on('click',()=>window.ReactNativeWebView.postMessage(JSON.stringify({type:'member-selected',memberId:member.id})));
